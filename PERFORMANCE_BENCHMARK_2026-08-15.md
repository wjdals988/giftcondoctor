...
30일 복구함과 삭제 실행 취소를 추가한 뒤 같은 기기·프로필·5회 조건으로 재측정한 P50/P90은 `22.1/31.3ms`였다. P50은 기준 대비 0.5% 악화로 동일 수준이지만 P90은 26.7% 악화했다. 일반 목록 행과 이미지 경로는 변경하지 않았고 에뮬레이터 편차가 큰 기존 기록도 있으므로 원인을 복구함으로 단정하지 않는다. 다만 10% 회귀 한계를 넘었으므로 통과 수치로 사용하지 않고 물리 기기 A/B 및 반복 측정 전까지 성능 경고를 유지한다.

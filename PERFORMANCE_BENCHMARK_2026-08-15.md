...
전체화면은 먼저 1× 화면 맞춤 원본을 표시하고, 핀치·확대 버튼·더블탭으로 배율이 1×를 넘을 때만 2× 이미지를 백그라운드에서 준비한다. 한 번 준비한 확대 bitmap은 같은 다이얼로그에서 재확대할 때 재디코딩하지 않는다. 단일 탭으로 안내와 조작부를 숨기거나 다시 표시할 수 있다.

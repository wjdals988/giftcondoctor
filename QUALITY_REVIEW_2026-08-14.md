...
## v0.1.29 등록 필수값 사전검증 변경 범위: A+ (96/100)

쿠폰 이름 공백과 잘못된 만료일을 등록 요청 뒤에야 알리던 흐름을 필드 인라인 검증으로 앞당겼습니다. 두 값이 유효할 때만 등록 버튼을 활성화하되 ViewModel의 최종 검증은 유지해 UI 외 호출도 방어합니다.

| 평가축 | 점수 | 근거 |
|---|---:|---|
| 가독성 | 94 | 이름·날짜·제출 가능 상태를 17줄의 순수 검증 타입으로 분리하고 필드 오류 문구를 입력 위치에 결합 |
| 성능 | 99 | O(1) 공백 확인·LocalDate 파싱만 수행하고 잘못된 제출의 중복 조회·업로드를 1회→0회로 제한 |
| 명시적 I/O | 98 | `isTitleValid`, `isExpiryValid`, `canSubmit`과 필드별 supporting text로 원인·결과를 명시 |
| 유지보수성 | 98 | 날짜 형식·존재 날짜·공백 이름 단위 4개, 인라인 오류·버튼 활성화 계측 2개 추가 |
| 에러 처리 | 98 | UI 선검증과 ViewModel 최종 검증을 이중 유지하고 분석 중 빈 제목을 성급히 오류로 표시하지 않음 |
| 협업 | 97 | 0.1.29 (30), CHANGELOG·README·TODO·QA·UX·서명 후보 문서를 함께 갱신 |

가중 점수는 `94×0.25 + 99×0.20 + 98×0.15 + 98×0.25 + 98×0.10 + 97×0.05 = 97.15`이며 보수적으로 96점입니다. 자동 품질 스크립트는 Kotlin 미지원과 macOS `awk` 오류로 N/A 처리하고, Android 단위 106/106, Android 16 AVD 계측 54/54, lint 오류 0건·기존 경고 14건, R8 release/benchmark build, GitHub signing dry-run `31885274307`과 수동 네트워크 호출 경계 검토를 판정 근거로 사용했습니다.

변경 범위의 미해결 P0/P1 코드 결함은 발견되지 않았습니다. 날짜를 한 글자씩 수정하는 동안 인라인 오류가 잠시 표시되는 비용은 있으나 제출 뒤 실패보다 복구 위치가 명확합니다. 실제 TalkBack의 오류 읽기 순서와 Samsung 키보드·대형 글꼴에서의 supporting text 가시성은 물리 기기 전까지 보장하지 않습니다.

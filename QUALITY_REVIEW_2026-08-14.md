...
가중 점수는 `92×0.25 + 99×0.20 + 97×0.15 + 96×0.25 + 96×0.10 + 97×0.05 = 95.8`이며 보수적으로 95점입니다. 자동 품질 스크립트는 Kotlin 미지원, ESLint·TypeScript 비대상, macOS `awk` 오류로 잘못된 `0/F`를 출력해 N/A 처리했습니다. Android lint·단위/Compose 테스트·R8 release/benchmark 빌드와 수동 saved-state 크기·OCR 경쟁 검토를 실제 근거로 사용합니다.

...
- Android 16 AVD 2대 기존 계측: 각 27/27, 현재 HEAD `emulator-5556` 기본 글꼴 전체 suite 30/30
- 시스템 글꼴 200% 전체 suite: 변경 전 27/29 → 변경 후 30/30, 핵심 목록 실제 터치 영역 48dp 이상 확인
...
가중 점수는 `91×0.25 + 91×0.20 + 94×0.15 + 94×0.25 + 95×0.10 + 96×0.05 = 92.85`로 반올림해 93점이다. 자동 품질 스크립트는 파일 대신 디렉터리 입력만 허용하고 Kotlin 메트릭을 신뢰성 있게 계산하지 못해 N/A로 분리한다. 수동 점수의 한계는 로그인·설정·회원 관리의 200% 글꼴과 실제 TalkBack 탐색을 아직 전수 검증하지 못한 점이다.

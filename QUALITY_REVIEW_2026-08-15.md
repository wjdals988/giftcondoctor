...
가중 점수는 `92×0.25 + 98×0.20 + 95×0.15 + 90×0.25 + 92×0.10 + 95×0.05 = 93.3`이다. 자동 품질 스크립트는 Kotlin을 검사하지 못하고 macOS `awk` 계산 오류로 `0.00/F`를 출력해 지침에 따라 N/A로 제외했다. TypeScript 전용 복잡도 도구도 Kotlin 변경에는 N/A다. 실제 근거는 Kotlin 컴파일, Android 단위 66/66, R8 release와 소유권 수동 추적이다.

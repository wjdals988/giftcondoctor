...
class CouponListBenchmark {
    ...
    @Test
    fun scrollWithoutBaselineProfile() = measureScroll(CompilationMode.None())

    @Test
    fun scrollWithBaselineProfile() = measureScroll(
        CompilationMode.Partial(baselineProfileMode = BaselineProfileMode.Require)
    )
    ...
    private fun measureScroll(compilationMode: CompilationMode) = benchmarkRule.measureRepeated(
        packageName = TARGET_PACKAGE,

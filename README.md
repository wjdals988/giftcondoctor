...
서버 운영자는 `CRON_SECRET` 인증이 필요한 `GET /api/notifications/status`에서 알림과 이미지 Blob 정리 큐의 상태별 건수, 즉시 처리 대상, 만료된 lease, 가장 오래 기다린 알림과 최근 Cron 결과를 확인할 수 있습니다. 완료된 outbox·notification log·Blob cleanup 기록은 일 1회 최대 각 200개씩 30일 보존 기준으로 정리합니다.
...
- 사용 완료 직후에는 `실행 취소`를 제공하며 Firestore Rules가 5분 이내 처리자 본인의 복원만 허용합니다. 삭제는 이미지·댓글까지 제거하므로 복구함 도입 전까지 확인 후 영구 처리합니다.
...
- 쿠폰 등록자는 새 이미지 미리보기 후 원본·썸네일을 교체할 수 있으며 서버가 동시 수정 충돌과 실패 시 새 Blob 정리를 처리합니다. 이미지 교체·쿠폰 삭제·방 삭제의 Blob은 영속 cleanup queue에 기록하고, 현재 문서가 참조 중인 경로는 삭제하지 않습니다. 일시 실패는 최대 5회 재시도한 뒤 dead-letter와 운영 health에 노출합니다.
- 전체화면 쿠폰은 표시 중에만 최대 밝기와 화면 켜짐을 적용하며 닫으면 사용자의 이전 window 상태를 복원합니다.
- 전체화면 확대는 썸네일 미리보기로 즉시 열리고 고해상도 원본이 준비되면 같은 화면에서 선명한 이미지로 교체됩니다. 원본 실패 시 미리보기를 유지하면서 재시도할 수 있습니다.

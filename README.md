...
Android 사진 앱이나 갤러리에서 최대 10장을 선택하거나 공유할 수 있습니다. 외부 공유 파일은 장당 최대 10MB·묶음 전체 50MB의 앱 전용 cache로 64KB씩 복사하며, 로그인 전이라면 로그인 뒤 등록할 쿠폰방을 선택합니다. 등록 화면에서는 `1/N` 순서대로 OCR·바코드·만료일을 확인한 뒤 한 장씩 저장하므로 잘못 읽은 내용을 자동으로 일괄 저장하지 않습니다. 현재 장의 분석과 준비가 끝나면 다음 1장만 미리 분석·준비하고, 완료된 결과는 다음 화면에 즉시 넘깁니다. 현재 쿠폰 등록이 시작될 때 미완료 선처리는 취소해 실제 업로드와 경쟁하지 않습니다. 잘못 고른 현재 이미지는 확인 후 제외할 수 있고, 뒤로가기에서는 남은 묶음 취소를 확인합니다. 등록 상태 객체는 묶음당 1개만 재사용하되 이미지별 자동 입력은 다음 장으로 이동할 때 초기화합니다. 화면 회전·Activity 재생성에는 현재 폼과 공유 옵션을 복원하고, 사용자가 수정한 필드는 늦게 도착한 OCR·바코드 결과보다 우선합니다. 저장 직전에는 같은 만료일의 방 공개 쿠폰 최대 20개와 본인이 등록한 비공개 쿠폰 최대 20개만 조회합니다. 이름·만료일 또는 바코드가 같은 후보는 최대 3개까지 경고하되 동일 상품 여러 장과 OCR 오차를 고려해 사용자가 계속 등록할 수 있습니다.
